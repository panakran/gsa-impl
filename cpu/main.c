...
int main( int argc, char **argv ){
    ...
    double *position, *velocity, *acceleration, *fitness, *mass, *inertia_mass, *force, *best_x;
    ...
    init_gsa( n_pop, n_dim, min, max, velocity, position );
    ...
    while( i<max_i ){
        ...
        // Update agents
        update_agents( n_pop, n_dim, g, force, min, max, velocity, position );
        //JOIN
    ...
    free( position ); free( velocity ); free( acceleration );free( fitness );
    free( mass ); free( inertia_mass ); free( force ); free( best_x );
    return 0;
}
